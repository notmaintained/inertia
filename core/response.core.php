 ...
 */
	...
	function valid_response_($response)
	{
        ...
        if ($handler_function_without_explicit_return) $response = '';
	...
	{
        $response = prepare_external_response_($response);
        flush_http_status_($response['status_code']);
        flush_headers_($response['headers']);
        echo $response['body'];
	}
		...
		function flush_headers_($headers)
		{
